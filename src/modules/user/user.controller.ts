...
export const registerUserHandler = async (
	...
	try {
		...
		console.log(e);
		return reply.code(500).send({ message: serverMessages.internalServerError });
	}
};
...
export const authUserLocalHandler = async (
	...
	try {
		...
		if (!user) {
			return reply.code(401).send({ message: serverMessages.invalidCredentials });
		}
		...
		reply.code(200).send({ email: user.email, name: user.name, roles: user.roles });
	} catch (e) {
		console.log(e);
		return reply.code(500).send({ message: serverMessages.internalServerError });
	}
...
export const refreshSessionHandler = async (request: FastifyRequest, reply: FastifyReply) => {
